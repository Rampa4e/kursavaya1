...
void VClient::auth()
{
    ...
    string hash = ::md5(data); // Используем функцию md5
...
vector<int32_t> VClient::calc(vector<vector<int32_t>> data)
{
    ...
    for (const auto &vec : data)
    {
        ...
        if (send(this->sock, vec.data(), vec_size * sizeof(int32_t), 0) < 0)
        {
    ...
    vector<int32_t> results(num_vectors);
    for (uint32_t i = 0; i < num_vectors; ++i)
    {
        if (recv(this->sock, &results[i], sizeof(int32_t), 0) < 0)
        {
