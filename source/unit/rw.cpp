...
// Метод для записи
void ReadWrite::writeVec(const vector<int32_t> &data)
{
    ofstream outfile(this->out, ios::binary);
    if (!outfile.is_open())
    {
    ...
    outfile.write(reinterpret_cast<const char *>(data.data()), vec_size * sizeof(int32_t));
    if (!outfile)
    {
    ...
    cout << "log: write vector comlete.\nvec: ";
    printVector(data);
}

// Метод для чтения
vector<vector<int32_t>> ReadWrite::readVec()
{
...
// Функция для вывода вектора int32_t
